...
int main()
{
    ...
    lightingShader.setInt("material.diffuse", METERIAL_TEXTURE_ID::DIFFUSE_ID);
    // 不同的材质对镜面光照的反射的情况不同
    // 例如木板对于镜面光照的强度不高，但是金属对于镜面广告的强度要比木板高很多
    lightingShader.setInt("material.specular", METERIAL_TEXTURE_ID::SPECULAR_ID);
    // 有一些物体的发光不受到其他光照的影响
    lightingShader.setInt("material.emission", METERIAL_TEXTURE_ID::EMISSION_ID);
    ...
    while (!glfwWindowShouldClose(mainWindow) && !glfwWindowShouldClose(imguiWindow))
    {
        ...
        // 绑定 漫反射 map
        glActiveTexture(GL_TEXTURE0 + METERIAL_TEXTURE_ID::DIFFUSE_ID);
        glBindTexture(GL_TEXTURE_2D, diffuseMap);

        // 绑定 镜面反射贴图
        glActiveTexture(GL_TEXTURE0 + METERIAL_TEXTURE_ID::SPECULAR_ID);
        glBindTexture(GL_TEXTURE_2D, specularMap);

        // 绑定 自发光贴图
        glActiveTexture(GL_TEXTURE0 + METERIAL_TEXTURE_ID::EMISSION_ID);
