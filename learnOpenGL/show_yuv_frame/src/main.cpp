...
std::vector<unsigned char> y_plane(video_width * video_height);
std::vector<unsigned char> u_plane(video_width / 2 * video_height / 2);
std::vector<unsigned char> v_plane(video_height / 2 * video_width / 2);
...
static const std::string yuv_file_path = "../res/yuv/output.yuv";
...
int main(int argc, char* argv[]) {
    ...
    // load yuv data
    readYUV420p(yuv_file_path, y_plane, u_plane, v_plane, video_width, video_height);
    // generate texture
    GLuint texY = createTexture(GL_RED, video_width, video_height);
    GLuint texU = createTexture(GL_RED, video_width / 2, video_height / 2);
    GLuint texV = createTexture(GL_RED, video_width / 2, video_height / 2);
    // upload texture data
    glBindTexture(GL_TEXTURE_2D, texY);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, video_width, video_height, GL_RED, GL_UNSIGNED_BYTE, y_plane.data());
    glBindTexture(GL_TEXTURE_2D, texU);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, video_width / 2, video_height / 2, GL_RED, GL_UNSIGNED_BYTE, u_plane.data());
    glBindTexture(GL_TEXTURE_2D, texV);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, video_width / 2, video_height / 2, GL_RED, GL_UNSIGNED_BYTE, v_plane.data());
    ...
    Shader shader{"../res/glsl/show_yuv_frame.vs", "../res/glsl/show_yuv_frame.fs"};
    shader.use();
