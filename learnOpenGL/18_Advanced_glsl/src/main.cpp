...
#define ROTATE_BEFORE_TRANSLATE_DEBUG
...
int main()
{
    ...
    if (image != nullptr)
    {
        ...
        spdlog::debug("Image Size is [{}, {}, {}]", width, height, nrChannels);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
#ifndef ROTATE_BEFORE_TRANSLATE_DEBUG
            // 先旋转再位移和先位移再旋转是完全不同的作用效果。
            model = glm::translate(base, cubePositions[i]);
            float angle = 20.0f * i;
            model = glm::rotate(model, glm::radians(angle), glm::vec3(0.5f, 0.0f, 0.0f));
#else  /* ROTATE_BEFORE_TRANSLATE_DEBUG */
            float angle = 20.0f * i;
            model = glm::rotate(base, glm::radians(angle), glm::vec3(0.5f, 0.0f, 0.0f));
            model = glm::translate(model, cubePositions[i]);
#endif /* ROTATE_BEFORE_TRANSLATE_DEBUG */
            shader.setMat4("model", model);
