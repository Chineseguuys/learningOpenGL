...
Shader::Shader(const char *vertexPath, const char *fragmentPath, const char *geometryPath)
{
    ...
    try {
        ...
        spdlog::error("{}: ERROR:SHADER::FILE_NOT_SUCCESSFULLY_READ, what={}", __FUNCTION__, e.what());
    }
...
void Shader::checkCompileErrors(GLuint shader, const std::string& type) {
    ...
    if (type != "PROGRAM") {
        ...
        if (!success) {
            ...
            spdlog::error("{}: ERROR:SHADER_COMPILATION_ERROR of type: {}", __FUNCTION__, type);
            spdlog::error("{}: error info log: {}", __FUNCTION__, infoLog);
        }
    } else {
        ...
        if (!success) {
            ...
            spdlog::error("{}: ERROR:PROGRAM_LINKING_ERROR of type: {}", __FUNCTION__ , type);
            spdlog::error("{}: error info log: {}", __FUNCTION__ ,infoLog);
        }
    }
}
...
Shader::~Shader() {
    if (this->ID != 0) {
	  ...
	  this->ID = 0;
		...
		spdlog::warn("{}: no program ID in the end", __FUNCTION__ );
	}
}
