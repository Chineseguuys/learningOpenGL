...
bool isPerspectivePressed = false;

bool useShadowBias = false;
bool useShadowBiasPressed = false;
...
int main(int argc, char* argv[]) {
    ...
    Shader shader("../res/glsl/shadow_mapping.vs", "../res/glsl/shadow_mapping.fs");
    Shader simpleDepthShader("../res/glsl/shadow_mapping_depth.vs", "../res/glsl/shadow_mapping_depth.fs");
    Shader debugDepthShader("../res/glsl/debug_quad.vs", "../res/glsl/debug_quad.fs");
    ...
    // load textures
    // -------------
    unsigned int woodTexture = loadTexture("../res/texture/wood.png", false);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        spdlog::trace("light projection:\n{}", lightProjection);
        lightView = glm::lookAt(lightPos, glm::vec3(0.0f), glm::vec3(0.0, 1.0, 0.0));
        spdlog::trace("light view:\n{}", lightView);
        ...
        shader.setBool("useShadowBias", useShadowBias);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, woodTexture);
        ...
        debugDepthShader.setBool("isPerspective", isPerspective);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, depthMap);
        //renderQuad();
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_G) ==GLFW_PRESS && !isPerspectivePressed) {
        isPerspective = !isPerspective;
        isPerspectivePressed = true;
        spdlog::debug("Key G has been pressed! Now isPerspective value is {}", isPerspective);
    }
    if (glfwGetKey(window, GLFW_KEY_G) == GLFW_RELEASE) {
        isPerspectivePressed = false;
    }

    if (glfwGetKey(window, GLFW_KEY_B) ==GLFW_PRESS && !useShadowBiasPressed) {
        useShadowBias = !useShadowBias;
        useShadowBiasPressed = true;
        spdlog::debug("Key B has been pressed! Now useShadowBias value is {}", useShadowBias);
    }
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_RELEASE) {
        useShadowBiasPressed = false;
    }
}
