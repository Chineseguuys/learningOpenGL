...
int main(int argc, char* argv[]) {
    ...
    // Load texture
    woodTexture = loadTexture("../res/texture/wood.png", false);

    // Configure depth cubemap texture
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 projection = glm::perspective(camera.mZoom, static_cast<float>(SCR_WIDTH) / static_cast<float>(SCR_HEIGHT), 0.1f, 100.0f);
