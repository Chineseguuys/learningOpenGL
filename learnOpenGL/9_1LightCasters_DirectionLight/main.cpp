...
// Camera
const static glm::vec3 lightPos(1.2f, 1.0f, 2.0f);        // 光源在世界坐标中的位置
const static glm::vec3 lightDirection(0.0f, 0.0f, -0.3f); // 平行光源的方向
...
int main()
{
    ...
    lightingShader.use();
    lightingShader.setInt("material.diffuse", MATERIAL_TEXTURE_ID::DIFFUSE_ID);
    // 不同的材质对镜面光照的反射的情况不同
    // 例如木板对于镜面光照的强度不高，但是金属对于镜面广告的强度要比木板高很多
    lightingShader.setInt("material.specular", MATERIAL_TEXTURE_ID::SPECULAR_ID);
    ...
    while (!glfwWindowShouldClose(mainWindow) && !glfwWindowShouldClose(imguiWindow))
    {
        ...
        // 绑定 漫反射 map
        glActiveTexture(GL_TEXTURE0 + MATERIAL_TEXTURE_ID::DIFFUSE_ID);
        glBindTexture(GL_TEXTURE_2D, diffuseMap);

        // 绑定 镜面反射贴图
        glActiveTexture(GL_TEXTURE0 + MATERIAL_TEXTURE_ID::SPECULAR_ID);
        ...
        lightingShader.setVec3("light.direction", lightDirection); // 平行光源不再使用 lightPos 而是使用 direction

        // shining 描述了一种镜面反光的散射程度，值越大，光的反射越集中；反之，光的反射越发散
