...
void Shader::SetUniformMat4f(const std::string& name, const glm::mat4& matrix) {
    int location = GetUniformLocation(name);
    // 1 表示我们传入的是一个矩阵
    // GL_FALSE 是表示不需要对矩阵进行转置的操作。在 OpenGL 当中，默认的矩阵的存储模式为列主模式
    // 意思也就是说，在连续的内存空间当中，先存储矩阵的第一列，再存储矩阵的第二列，以此类推。在行主模式
    // 当中，在连续的内存空间当中，先存储矩阵的第一行，再存储矩阵的第二行，以此类推。 glm 采用的是列主
    // 模式来存储矩阵，所以它和 OpenGL 的矩阵存储模式是保持一致的，因此不需要进行转换
    // 函数的最后一个参数需要传递存储矩阵数据的原始数组。在这里传入矩阵的第一个元素的地址即可
