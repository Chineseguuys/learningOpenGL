...
int main(int argc, char* argv[]) {
    ...
    // 兼容模式将会为我们默认创建一个顶点数组对象，在这里使用的是核心模式
    ...
    // 注意顶点的偏移量值 
    // 对应 shader 中的 location 0
    // 函数的第一个参数和 shader 源程序中的 location 需要保持一致
    GLCALL( glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), static_cast<const void*>(0)) );
    GLCALL( glEnableVertexAttribArray(0) );

    // 一定要注意颜色的偏移量值 3 * sizeof(float)
    // 对应 shader 中的 location 1
    GLCALL( glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), reinterpret_cast<const void*>(3 * sizeof(float))) );
