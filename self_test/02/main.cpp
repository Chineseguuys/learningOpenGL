...
int main(int argc, char* argv[]) {
    ...
    GLCALL( glBufferData(GL_ARRAY_BUFFER, sizeof(firstTriangle), firstTriangle, GL_STATIC_DRAW) );
    ...
    GLCALL( glBufferData(GL_ARRAY_BUFFER, sizeof(secondTriangle), secondTriangle, GL_STATIC_DRAW) );
    GLCALL( glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), static_cast<const char*>(0)) );

    GLCALL( glEnableVertexAttribArray(0) );
    GLCALL( glBindVertexArray(0) );

    // shader
    ShaderProgramSource shader_source1 = ParseShader("./res/shader1.shader");
    ShaderProgramSource shader_source2 = ParseShader("./res/shader2.shader");
    GLCALL( unsigned int shader1 = CreateShader(shader_source1.VertexSource, shader_source1.FragmentSource) );
    GLCALL( unsigned int shader2 = CreateShader(shader_source2.VertexSource, shader_source2.FragmentSource) );

    // 颜色变化
    float red_color = 0.2f;
    float green_color = 0.2f;
    float red_increment = 0.0125f;
    float green_increment = 0.0125f;

    int shader1_location, shader2_location;
    GLCALL( shader1_location = glGetUniformLocation(shader1, "u_color") );
    GLCALL( shader2_location = glGetUniformLocation(shader2, "u_color") );

    while(!glfwWindowShouldClose(window)) {
        glClear(GL_COLOR_BUFFER_BIT);

        GLCALL( glUseProgram(shader1) );
        GLCALL( glUniform3f(shader1_location, red_color, 0.2f, 0.5f) );
        GLCALL( glBindVertexArray(vaos[0]) );
        GLCALL( glDrawArrays(GL_TRIANGLES, 0, 3) );

        GLCALL( glUseProgram(shader2) );
        GLCALL( glUniform3f(shader2_location, 0.1f, 0.5f, green_color) );
        GLCALL( glBindVertexArray(vaos[1]) );
        GLCALL( glDrawArrays(GL_TRIANGLES, 0, 3) );
        ...
        red_color += red_increment;
        green_color += green_increment;
